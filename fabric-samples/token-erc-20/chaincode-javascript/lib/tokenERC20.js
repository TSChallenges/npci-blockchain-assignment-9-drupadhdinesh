...
class TokenERC20Contract extends Contract {
    ...
    */
    ...
    async Initialize(ctx, name, symbol, decimals) {
        // Check minter authorization - this sample assumes Rbi is the central banker with privilege to set Options for these tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'RbiMSP') {
    ...
    async Mint(ctx, amount) {
        ...
        // Check minter authorization - this sample assumes Rbi is the central banker with privilege to mint new tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'RbiMSP') {
    ...
    async Burn(ctx, amount) {
        ...
        // Check minter authorization - this sample assumes Rbi is the central banker with privilege to burn tokens
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'RbiMSP') {
            throw new Error('client is not authorized to mint new tokens');
        }
