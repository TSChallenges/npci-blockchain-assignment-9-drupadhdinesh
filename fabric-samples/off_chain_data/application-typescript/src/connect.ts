...
const peerName = 'peer0.Rbi.example.com';
const mspId = process.env.MSP_ID ?? 'RbiMSP';

// Path to crypto materials.
const cryptoPath = path.resolve(process.env.CRYPTO_PATH ?? path.resolve(__dirname, '..', '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'Rbi.example.com'));

// Path to user private key directory.
const keyDirectoryPath = path.resolve(process.env.KEY_DIRECTORY_PATH ?? path.resolve(__dirname, cryptoPath, 'users', 'User1@Rbi.example.com', 'msp', 'keystore'));

// Path to user certificate.
const certPath = path.resolve(process.env.CERT_PATH ?? path.resolve(__dirname, cryptoPath, 'users', 'User1@Rbi.example.com', 'msp', 'signcerts', 'cert.pem'));
