...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.Rbi.example.com Rbi
    peer_cert admin Admin@Rbi.example.com Rbi

    infoln "Creating Hdfc Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.Hdfc.example.com Hdfc
    peer_cert admin Admin@Hdfc.example.com Hdfc
    
    infoln "Creating Sbi Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.Sbi.example.com Sbi
    peer_cert admin Admin@Sbi.example.com Sbi
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/Rbi/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    while [[ $rc -ne 0 && $COUNTER -lt $MAX_RETRY ]]; do
      sleep 1
      set -x
      fabric-ca-client getcainfo -u https://admin:adminpw@localhost:7054 --caname ca-Rbi --tls.certfiles "${PWD}/organizations/fabric-ca/Rbi/ca-cert.pem"
      res=$?
    { set +x; } 2>/dev/null
    rc=$res  # Update rc
    COUNTER=$((COUNTER + 1))
  ...
  infoln "Generating CCP files for Rbi and Hdfc and Sbi"
...
# call the script to create the channel, join the peers of Rbi and Hdfc,
...
function networkDown() {
  ...
  # stop Sbi containers also in addition to Rbi and Hdfc, in case we were running sample to add Sbi
  # COMPOSE_Sbi_BASE_FILES="-f addSbi/compose/${COMPOSE_FILE_Sbi_BASE} -f addSbi/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_Sbi_BASE}"
  # COMPOSE_Sbi_COUCH_FILES="-f addSbi/compose/${COMPOSE_FILE_Sbi_COUCH} -f addSbi/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_Sbi_COUCH}"
  # COMPOSE_Sbi_CA_FILES="-f addSbi/compose/${COMPOSE_FILE_Sbi_CA} -f addSbi/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_Sbi_CA}"
  # COMPOSE_Sbi_FILES="${COMPOSE_Sbi_BASE_FILES} ${COMPOSE_Sbi_COUCH_FILES} ${COMPOSE_Sbi_CA_FILES}"

  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} down --volumes --remove-orphans
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
    ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_Sbi_FILES} down --volumes
  else
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.Rbi.example.com docker_peer0.Hdfc.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/Rbi/msp organizations/fabric-ca/Rbi/tls-cert.pem organizations/fabric-ca/Rbi/ca-cert.pem organizations/fabric-ca/Rbi/IssuerPublicKey organizations/fabric-ca/Rbi/IssuerRevocationPublicKey organizations/fabric-ca/Rbi/fabric-ca-server.db'
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addSbi/fabric-ca/Sbi/msp addSbi/fabric-ca/Sbi/tls-cert.pem addSbi/fabric-ca/Sbi/ca-cert.pem addSbi/fabric-ca/Sbi/IssuerPublicKey addSbi/fabric-ca/Sbi/IssuerRevocationPublicKey addSbi/fabric-ca/Sbi/fabric-ca-server.db'
...
# use this as the default docker-compose yaml definition for Sbi
COMPOSE_FILE_Sbi_BASE=compose-Sbi.yaml
# use this as the docker compose couch file for Sbi
COMPOSE_FILE_Sbi_COUCH=compose-couch-Sbi.yaml
# certificate authorities compose file
COMPOSE_FILE_Sbi_CA=compose-ca-Sbi.yaml
#
