...
```
./network.sh deployCC -ccn auction -ccp ../auction-simple/chaincode-go/ -ccl go -ccep "OR('RbiMSP.peer','HdfcMSP.peer')"
```
...
Run the following command to register and enroll the seller identity that will create the auction. The seller will belong to Rbi.
```
node registerEnrollUser.js Rbi seller
```

You should see the logs of the seller wallet being created as well. Run the following commands to register and enroll 2 bidders from Rbi and another 2 bidders from Hdfc:
```
...
```
node createAuction.js Rbi seller PaintingAuction painting
```
...
```
node bid.js Rbi bidder1 PaintingAuction 800
```
...
*** Result:  Bid: {
  ...
  "price": 800,
  "org": "RbiMSP",
  "bidder": "x509::CN=bidder1,OU=client+OU=Rbi+OU=department1::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US"
}
```

The bid is stored in the Rbi implicit data collection. The `"bidder"` parameter is the information from the certificate of the user that created the bid. Only this identity will be able can query the bid from private state or reveal the bid during the auction.
...
```
node submitBid.js Rbi bidder1 PaintingAuction $BIDDER1_BID_ID
```
...
```
node bid.js Rbi bidder2 PaintingAuction 500
```
...
```
node submitBid.js Rbi bidder2 PaintingAuction $BIDDER2_BID_ID
```
...
```
node bid.js Hdfc bidder3 PaintingAuction 700
```
...
```
node submitBid.js Hdfc bidder3 PaintingAuction $BIDDER3_BID_ID
```
...
Now that a bid from Hdfc has been added to the auction, any updates to the auction need to be endorsed by the Hdfc peer. The applications will use `"organizations"` field to specify which organizations need to endorse submitting a new bid, revealing a bid, or updating the auction status.

### Bid as bidder4

Bidder4 from Hdfc would like to purchase the painting for 900 dollars:
```
node bid.js Hdfc bidder4 PaintingAuction 900
```
...
```
node submitBid.js Hdfc bidder4 PaintingAuction $BIDDER4_BID_ID
```
...
```
node closeAuction.js Rbi seller PaintingAuction
```
...
```
node revealBid.js Rbi bidder1 PaintingAuction $BIDDER1_BID_ID
```
...
Bidder3 from Hdfc will also reveal their bid:
```
node revealBid.js Hdfc bidder3 PaintingAuction $BIDDER3_BID_ID
```

If the auction ended now, Bidder1 would win. Let's try to end the auction using the seller identity and see what happens.

```
node endAuction.js Rbi seller PaintingAuction
```
...
Instead of ending the auction, the transaction results in an endorsement policy failure. The end of the auction needs to be endorsed by Hdfc. Before endorsing the transaction, the Hdfc peer queries its private data collection for any winning bids that have not yet been revealed. Because Bidder4 created a bid that is above the winning price, the Hdfc peer refuses to endorse the transaction that would end the auction.

Before we can end the auction, we need to reveal the bid from bidder4.
```
node revealBid.js Hdfc bidder4 PaintingAuction $BIDDER4_BID_ID
```

Bidder2 from Rbi would not win the auction in either case. As a result, Bidder2 decides not to reveal their bid.

## End the auction

Now that the winning bids have been revealed, we can end the auction:
```
node endAuction Rbi seller PaintingAuction
```

The transaction was successfully endorsed by both Rbi and Hdfc, who both calculated the same price and winner. The winning bidder is listed along with the price:
```
*** Result: Auction: {
  "objectType": "auction",
  "item": "painting",
  ...
  "revealedBids": {
    "\u0000bid\u0000PaintingAuction\u000019a7a0dd2c5456a3f79c2f9ccb09dddd0f1c9ece514dfea7cbea06e7cbc79855\u0000": {
      ...
      "price": 900,
      "org": "HdfcMSP",
      "bidder": "x509::CN=bidder4,OU=client+OU=Hdfc+OU=department1::CN=ca.Hdfc.example.com,O=Hdfc.example.com,L=Hursley,ST=Hampshire,C=UK"
    },
    "\u0000bid\u0000PaintingAuction\u00005c049b0b4552d34c88e0f8fb5abca31fa04472b7e1336a16650ac8cfb0b16472\u0000": {
      "objectType": "bid",
      "price": 800,
    ...
    "\u0000bid\u0000PaintingAuction\u00005ee4fa53b54ea0821e57a6884a1ada5eb04f136ee222e92d7399bcdf47556ea1\u0000": {
      ...
      "price": 700,
      "org": "HdfcMSP",
      "bidder": "x509::CN=bidder3,OU=client+OU=Hdfc+OU=department1::CN=ca.Hdfc.example.com,O=Hdfc.example.com,L=Hursley,ST=Hampshire,C=UK"
    }
  },
  "winner": "x509::CN=bidder4,OU=client+OU=Hdfc+OU=department1::CN=ca.Hdfc.example.com,O=Hdfc.example.com,L=Hursley,ST=Hampshire,C=UK",
  "price": 900,
  "status": "ended"
}
```
