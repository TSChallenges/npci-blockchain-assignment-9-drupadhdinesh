...
Open terminal windows for 3 ordering nodes or 4 if running BFT Consensus, 4 peer nodes, and 4 peer admins as seen in the following terminal setup. The first two peers and peer admins belong to Rbi, the latter two peer and peer admins belong to Hdfc.
Note, you can start with two ordering nodes and a single Rbi peer node and single Rbi peer admin terminal if you would like to keep things even more minimal (two ordering nodes are required to achieve Raft consensus (2 of 3), while a single peer from Rbi can be utilized since the endorsement policy is set as any single organization).
...
- cd to the `test-network-nano-bash` directory in each terminal window
- Before running the  `./generate_artifacts.sh -ca` in the first orderer terminal, run `./ordererca.sh`, `./Rbica.sh`, `./Hdfcca.sh` in the repsective terminals.
