...
class TokenERC721Contract extends Contract {
    ...
    */
    ...
    async Initialize(ctx, name, symbol) {
        ...
        // Check minter authorization - this sample assumes Rbi is the issuer with privilege to initialize contract (set the name and symbol)
    ...
    */
    async MintWithTokenURI(ctx, tokenId, tokenURI) {
        ...
        // Check minter authorization - this sample assumes Rbi is the issuer with privilege to mint a new token
        const clientMSPID = ctx.clientIdentity.getMSPID();
        if (clientMSPID !== 'RbiMSP') {
