...
In this example, there are two organizations (org). We will register new identities using the Rbi and Hdfc Certificate Authorities (CA's), and then use the CA's to generate each identity's certificate and private key.
...
The terminal we have been using will represent Rbi. We will use the Rbi CA to create a new identity. Set the Fabric CA client home to the MSP of the Rbi CA admin (this identity was generated by the test network script).

```bash
export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/peerOrganizations/Rbi.example.com/
```

Register `Person1` identity to Rbi.
```bash
fabric-ca-client register --caname ca-Rbi --id.name person1 --id.secret person1pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Rbi/tls-cert.pem"
```

Generate the identity certificates and MSP folder.

```bash
fabric-ca-client enroll -u https://person1:person1pw@localhost:7054 --caname ca-Rbi -M "${PWD}/organizations/peerOrganizations/Rbi.example.com/users/person1@Rbi.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Rbi/tls-cert.pem"
```

Copy the Node OU configuration file into the identity MSP folder.

```bash
cp "${PWD}/organizations/peerOrganizations/Rbi.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Rbi.example.com/users/person1@Rbi.example.com/msp/config.yaml"
```

Open a new terminal to represent Hdfc and navigate to fabric-samples/test-network. We'll use the Hdfc CA to create the Hdfc identities. Set the Fabric CA client home to the MSP of the Hdfc CA admin.
...
Register `Person2`, `Person3`, `Person4`, `Person5` to Hdfc.

```bash
fabric-ca-client register --caname ca-Hdfc --id.name person2 --id.secret person2pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Hdfc/tls-cert.pem"
fabric-ca-client enroll -u https://person2:person2pw@localhost:8054 --caname ca-Hdfc -M "${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person2@Hdfc.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Hdfc/tls-cert.pem"
cp "${PWD}/organizations/peerOrganizations/Hdfc.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person2@Hdfc.example.com/msp/config.yaml"

fabric-ca-client register --caname ca-Hdfc --id.name person3 --id.secret person3pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Hdfc/tls-cert.pem"
fabric-ca-client enroll -u https://person3:person3pw@localhost:8054 --caname ca-Hdfc -M "${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person3@Hdfc.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Hdfc/tls-cert.pem"
cp "${PWD}/organizations/peerOrganizations/Hdfc.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person3@Hdfc.example.com/msp/config.yaml"

fabric-ca-client register --caname ca-Hdfc --id.name person4 --id.secret person4pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Hdfc/tls-cert.pem"
fabric-ca-client enroll -u https://person4:person4pw@localhost:8054 --caname ca-Hdfc -M "${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person4@Hdfc.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Hdfc/tls-cert.pem"
cp "${PWD}/organizations/peerOrganizations/Hdfc.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person4@Hdfc.example.com/msp/config.yaml"

fabric-ca-client register --caname ca-Hdfc --id.name person5 --id.secret person5pw --id.type client --tls.certfiles "${PWD}/organizations/fabric-ca/Hdfc/tls-cert.pem"
fabric-ca-client enroll -u https://person5:person5pw@localhost:8054 --caname ca-Hdfc -M "${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person5@Hdfc.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Hdfc/tls-cert.pem"
cp "${PWD}/organizations/peerOrganizations/Hdfc.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person5@Hdfc.example.com/msp/config.yaml"
```
...
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/Rbi.example.com/users/person1@Rbi.example.com/msp
...
```
x509::CN=person1,OU=client,O=Hyperledger,ST=North Carolina,C=US::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US
```
...
Switch to the Hdfc terminal and set the following environment variables.
```bash
export FABRIC_CFG_PATH=$PWD/../config/
export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID=HdfcMSP
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/Hdfc.example.com/users/person2@Hdfc.example.com/msp
