...
func (s *SmartContract) Mint(ctx contractapi.TransactionContextInterface, account string, id uint64, amount uint64) error {
	...
	// Check minter authorization - this sample assumes Rbiis the central banker with privilege to mint new tokens
...
func (s *SmartContract) Burn(ctx contractapi.TransactionContextInterface, account string, id uint64, amount uint64) error {
	...
	// Check minter authorization - this sample assumes Rbiis the central banker with privilege to burn new tokens
	err = authorizationHelper(ctx)
	if err != nil {
		return err
	}
...
func (s *SmartContract) Initialize(ctx contractapi.TransactionContextInterface, name string, symbol string) (bool, error) {
	...
	// Check minter authorization - this sample assumes Rbiis the central banker with privilege to intitialize contract
...
// authorizationHelper checks minter authorization - this sample assumes Rbiis the central banker with privilege to mint new tokens
