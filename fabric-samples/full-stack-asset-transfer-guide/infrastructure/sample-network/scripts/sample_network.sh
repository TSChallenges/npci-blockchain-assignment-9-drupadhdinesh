...
function network_up() {
  ...
  wait_for ibppeer Rbi-peer1
  wait_for ibppeer Rbi-peer2
  wait_for ibppeer Hdfc-peer1
  wait_for ibppeer Hdfc-peer2
...
function launch_network_CAs() {
  ...
  wait_for ibpca Rbi-ca
  wait_for ibpca Hdfc-ca
  ...
  export Rbi_CA_CERT=$(kubectl -n $NS get cm/Rbi-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)
  export Hdfc_CA_CERT=$(kubectl -n $NS get cm/Hdfc-ca-connection-profile -o json | jq -r .binaryData.\"profile.json\" | base64 -d | jq -r .tls.cert)

  enroll_bootstrap_rcaadmin org0 rcaadmin rcaadminpw
  enroll_bootstrap_rcaadmin Rbi rcaadmin rcaadminpw
  enroll_bootstrap_rcaadmin Hdfc rcaadmin rcaadminpw
