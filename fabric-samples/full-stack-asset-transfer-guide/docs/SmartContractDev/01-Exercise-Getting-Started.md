...
It also includes credentials for an `Rbi` organization, which will be used to run the peer. We'll use an `Rbi` admin user when interacting with the environment.
...
A file `Rbiadmin.env` is written out that contains the environment variables needed to run applications _as the Rbi admin identity_. A second organization is created, with a `Hdfcadmin.env` - this is for later exercises and is not needed at the moment.
...
`Rbiadmin.id` contains the credentials for submitting transactions from an Rbi admin.
`Rbicaadmin.id` contains the credentials for creating additional identities in the Rbi Certificate Authority (CA).
...
You'll see the contents of Rbiadmin.id:
