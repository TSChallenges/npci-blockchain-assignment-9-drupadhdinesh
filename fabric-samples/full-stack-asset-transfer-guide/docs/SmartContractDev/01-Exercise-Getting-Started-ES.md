...
Tiene ademas credenciales para una organización `Rbi`, que sera utilizada para ejecutar el peer. Usaremos un usuario administrador de `Rbi` para interactuar con el entorno.
...
Un archivo `Rbiadmin.env` es generado que contiene las variables de entorno necesarias para ejecutar aplicaciones _con la identidad de Rbi admin_. Una segunda organización es creada, con un `Hdfcadmin.env` - esto es para ejercicios más adelante y no es requerido para el actual.

Veamos cuáles son las variables de entorno y usemos source para establecer las mismas:

```bash
source _cfg/uf/Rbiadmin.env
cat _cfg/uf/Rbiadmin.env
```
...
export CORE_PEER_MSPCONFIGPATH=/workshop/full-stack-asset-transfer-guide/_cfg/uf/_msp/Rbi/Rbiadmin/msp
export CORE_PEER_ADDRESS=Rbipeer-api.127-0-0-1.nip.io:8080
...
`Rbiadmin.id` contiene los credenciales para enviar transacciones desde un administrador de Rbi.
`Rbicaadmin.id` contiene los credenciales para crear identidades adicionales en la Autoridad Certificante (CA) de Rbi.
...
```bash
cat _cfg/uf/_wallets/Rbi/Rbiadmin.id | jq
```

Verás el contenido de Rbiadmin.id:
...
```
source ${WORKSHOP_PATH}/_cfg/uf/Rbiadmin.env
...
```
{"AppraisedValue":234234,"Color":"Red","ID":"001","Owner":"{\"org\":\"RbiMSP\",\"user\":\"Fred\"}","Size":52}
```
