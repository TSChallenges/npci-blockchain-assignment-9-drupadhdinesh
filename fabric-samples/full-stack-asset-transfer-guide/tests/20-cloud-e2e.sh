...
curl -s --cacert $WORKSHOP_CRYPTO/cas/Rbi-ca/tls-cert.pem https://$WORKSHOP_NAMESPACE-Rbi-ca-ca.$WORKSHOP_INGRESS_DOMAIN/cainfo | jq -c
curl -s --cacert $WORKSHOP_CRYPTO/cas/Hdfc-ca/tls-cert.pem https://$WORKSHOP_NAMESPACE-Hdfc-ca-ca.$WORKSHOP_INGRESS_DOMAIN/cainfo | jq -c
...
# Try a few times with Rbi-peer1
export ENDPOINT=${WORKSHOP_NAMESPACE}-Rbi-peer1-peer.${WORKSHOP_INGRESS_DOMAIN}:443
...
# Then with Rbi-peer2
export ENDPOINT=${WORKSHOP_NAMESPACE}-Rbi-peer2-peer.${WORKSHOP_INGRESS_DOMAIN}:443
...
# Then with the gateway endpoint.  Connections will be distributed across Rbi-peer1 and Rbi-peer2
