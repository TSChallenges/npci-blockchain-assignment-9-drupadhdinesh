...
To set the path within the peer container, you will need to modify the docker compose file to mount a couple of additional volumes. Open the file `test-network/compose/docker/docker-compose-test-net.yaml`, and add to the `volumes` section of both `peer0.Rbi.example.com` and `peer0.Hdfc.example.com` the following two lines:
...
Run the following commands to install the `asset-transfer-basic-external.tar.gz` chaincode on Rbi. The `setGlobals` function simply sets the environment variables that allow you to act as Rbi or Hdfc.
...
Install the chaincode package on the Hdfc peer:
...
```
setGlobals 1
peer lifecycle chaincode queryinstalled --peerAddresses localhost:7051 --tlsRootCertFiles organizations/peerOrganizations/Rbi.example.com/peers/peer0.Rbi.example.com/tls/ca.crt
```
...
```
docker run -it --rm --name asset-transfer-basic.Rbi.example.com --hostname asset-transfer-basic.Rbi.example.com --env-file chaincode.env --network=fabric_test hyperledger/asset-transfer-basic
```
...
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "$PWD/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" --channelID mychannel --name basic --peerAddresses localhost:7051 --tlsRootCertFiles "$PWD/organizations/peerOrganizations/Rbi.example.com/peers/peer0.Rbi.example.com/tls/ca.crt" --peerAddresses localhost:9051 --tlsRootCertFiles organizations/peerOrganizations/Hdfc.example.com/peers/peer0.Hdfc.example.com/tls/ca.crt --version 1.0 --sequence 1
```
...
```
openssl req -nodes -x509 -newkey rsa:4096 -keyout crypto/key1.pem -out crypto/cert1.pem -subj "/C=IN/ST=KA/L=Bangalore/O=example Inc/OU=Developer/CN=asset-transfer-basic.Rbi.example.com/emailAddress=dev@asset-transfer-basic.Rbi.example.com"
```

For `Hdfc.example.com`

```
openssl req -nodes -x509 -newkey rsa:4096 -keyout crypto/key2.pem -out crypto/cert2.pem -subj "/C=IN/ST=KA/L=Bangalore/O=example Inc/OU=Developer/CN=asset-transfer-basic.Hdfc.example.com/emailAddress=dev@asset-transfer-basic.Hdfc.example.com"
```

- Copy the CA file contents for both `Rbi.example.com` & `Hdfc.example.com`

```
cp ../../test-network/organizations/peerOrganizations/Rbi.example.com/ca/ca.Rbi.example.com-cert.pem crypto/rootcert1.pem
cp ../../test-network/organizations/peerOrganizations/Hdfc.example.com/ca/ca.Hdfc.example.com-cert.pem crypto/rootcert2.pem
```
...
```
{
  "address": "asset-transfer-basic.Rbi.example.com:9999",
  "dial_timeout": "10s",
  "tls_required": true,
...
- Follow the instructions in [Package](#packaging-and-installing-chaincode) and [Install](#installing-the-external-chaincode) steps for each organization. Remember that the chaincode server's address for the second organization is `asset-transfer-basic.Hdfc.example.com:9999`.
