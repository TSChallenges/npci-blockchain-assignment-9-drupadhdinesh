...
/**
 * Creates an in memory wallet to hold credentials for an Rbi and Hdfc user
 *
...
export const createWallet = async (): Promise<Wallet> => {
    ...
    const RbiIdentity = {
        credentials: {
            certificate: config.certificateRbi,
            privateKey: config.privateKeyRbi,
        },
        mspId: config.mspIdRbi,
        type: 'X.509',
    };

    await wallet.put(config.mspIdRbi, RbiIdentity);

    const HdfcIdentity = {
        credentials: {
            certificate: config.certificateHdfc,
            privateKey: config.privateKeyHdfc,
        },
        mspId: config.mspIdHdfc,
        type: 'X.509',
    };

    await wallet.put(config.mspIdHdfc, HdfcIdentity);
