...
describe('Asset Transfer Besic REST API', () => {
    ...
    describe('/live', () => {
        it('GET should respond with 200 OK json', async () => {
            ...
            const mockRbiQsccContract = mock<Contract>();
            mockRbiQsccContract.evaluateTransaction
                .calledWith('GetChainInfo')
                .mockResolvedValue(mockBlockchainInfoBuffer);
            app.locals[config.mspIdRbi] = {
                qsccContract: mockRbiQsccContract,
            };

            const mockHdfcQsccContract = mock<Contract>();
            mockHdfcQsccContract.evaluateTransaction
                .calledWith('GetChainInfo')
                .mockResolvedValue(mockBlockchainInfoBuffer);
            app.locals[config.mspIdHdfc] = {
                qsccContract: mockHdfcQsccContract,
            };
