...
We will create the identities using the Rbi CA. Set the Fabric CA client home to the MSP of the Rbi CA admin:
```
...
```
fabric-ca-client register --id.name creator1 --id.secret creator1pw --id.type client --id.affiliation Rbi --id.attrs 'abac.creator=true:ecert' --tls.certfiles "${PWD}/organizations/fabric-ca/Rbi/tls-cert.pem"
```
...
```
fabric-ca-client enroll -u https://creator1:creator1pw@localhost:7054 --caname ca-Rbi -M "${PWD}/organizations/peerOrganizations/Rbi.example.com/users/creator1@Rbi.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Rbi/tls-cert.pem"
```
...
```
cp "${PWD}/organizations/peerOrganizations/Rbi.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Rbi.example.com/users/creator1@Rbi.example.com/msp/config.yaml"
```
...
```
fabric-ca-client register --id.name creator2 --id.secret creator2pw --id.type client --id.affiliation Rbi --id.attrs 'abac.creator=true:' --tls.certfiles "${PWD}/organizations/fabric-ca/Rbi/tls-cert.pem"
```

The following enroll command will add the attribute to the certificate:

```
fabric-ca-client enroll -u https://creator2:creator2pw@localhost:7054 --caname ca-Rbi --enrollment.attrs "abac.creator" -M "${PWD}/organizations/peerOrganizations/Rbi.example.com/users/creator2@Rbi.example.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/Rbi/tls-cert.pem"
```

Run the command below to copy the Node OU configuration file into the creator2 MSP folder.
```
cp "${PWD}/organizations/peerOrganizations/Rbi.example.com/msp/config.yaml" "${PWD}/organizations/peerOrganizations/Rbi.example.com/users/creator2@Rbi.example.com/msp/config.yaml"
```
...
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/Rbi.example.com/users/creator1@Rbi.example.com/msp
...
```
{"ID":"Asset1","color":"blue","size":20,"owner":"x509::CN=creator1,OU=client+OU=Rbi,O=Hyperledger,ST=North Carolina,C=US::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US","appraisedValue":100}
```

## Transfer the asset

As the owner of Asset1, the creator1 identity has the ability to transfer the asset to another owner. In order to transfer the asset, the owner needs to provide the name and issuer of the new owner to the `TransferAsset` function. The `asset-transfer-abac` smart contract has a `GetSubmittingClientIdentity` function that allows users to retrieve their certificate information and provide it to the asset owner out of band (we omit this step). Issue the command below to transfer Asset1 to the user1 identity from Rbi that was created when the test network was deployed:
```
export RECIPIENT="x509::CN=user1,OU=client,O=Hyperledger,ST=North Carolina,C=US::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US"
...
```
{"ID":"Asset1","color":"blue","size":20,"owner":"x509::CN=user1,OU=client,O=Hyperledger,ST=North Carolina,C=US::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US","appraisedValue":100}
```
...
```
{"ID":"Asset1","color":"green","size":20,"owner":"x509::CN=user1,OU=client,O=Hyperledger,ST=North Carolina,C=US::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US","appraisedValue":100}
```
