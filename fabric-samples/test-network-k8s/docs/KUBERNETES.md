...
kubectl create -f kube/pv-fabric-Rbi.yaml
kubectl create -f kube/pv-fabric-Hdfc.yaml

# Create persistent volume claims binding to the host (docker) volumes
kubectl -n $NS create -f kube/pvc-fabric-org0.yaml
kubectl -n $NS create -f kube/pvc-fabric-Rbi.yaml
kubectl -n $NS create -f kube/pvc-fabric-Hdfc.yaml
```
...
aliases (e.g. grpcs://Rbi-peer1.test-network.svc.cluster.local:443) and traverse private K8s network routes.
