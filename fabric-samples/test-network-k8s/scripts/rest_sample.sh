...
function construct_rest_sample_configmap() {
  ...
  echo "$(json_ccp 1 $peer_pem $ca_pem $Rbi_NS)" > build/fabric-rest-sample-config/HLF_CONNECTION_PROFILE_Rbi
  ...
  echo "$(json_ccp 2 $peer_pem $ca_pem $Hdfc_NS)" > build/fabric-rest-sample-config/HLF_CONNECTION_PROFILE_Hdfc

  cp $ENROLLMENT_DIR/Rbi/users/Rbiadmin/msp/signcerts/cert.pem $CONFIG_DIR/HLF_CERTIFICATE_Rbi
  cp $ENROLLMENT_DIR/Hdfc/users/Hdfcadmin/msp/signcerts/cert.pem $CONFIG_DIR/HLF_CERTIFICATE_Hdfc

  cp $ENROLLMENT_DIR/Rbi/users/Rbiadmin/msp/keystore/key.pem $CONFIG_DIR/HLF_PRIVATE_KEY_Rbi
  cp $ENROLLMENT_DIR/Hdfc/users/Hdfcadmin/msp/keystore/key.pem $CONFIG_DIR/HLF_PRIVATE_KEY_Hdfc
