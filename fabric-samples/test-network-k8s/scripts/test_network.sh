...
function launch_peers() {
  ...
  apply_template kube/Rbi/Rbi-peer1.yaml $Rbi_NS
  apply_template kube/Rbi/Rbi-peer2.yaml $Rbi_NS
  apply_template kube/Hdfc/Hdfc-peer1.yaml $Hdfc_NS
  apply_template kube/Hdfc/Hdfc-peer2.yaml $Hdfc_NS

  kubectl -n $Rbi_NS rollout status deploy/Rbi-peer1
  kubectl -n $Rbi_NS rollout status deploy/Rbi-peer2
  kubectl -n $Hdfc_NS rollout status deploy/Hdfc-peer1
  kubectl -n $Hdfc_NS rollout status deploy/Hdfc-peer2
...
function create_local_MSP() {
  ...
  create_peer_local_MSP Rbi peer1 $Rbi_NS
  create_peer_local_MSP Rbi peer2 $Rbi_NS

  create_peer_local_MSP Hdfc peer1 $Hdfc_NS
  create_peer_local_MSP Hdfc peer2 $Hdfc_NS
...
function stop_services() {
  ...
  for ns in $ORG0_NS $Rbi_NS $Hdfc_NS; do
