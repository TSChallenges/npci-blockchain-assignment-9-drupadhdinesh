...
function chaincode_command_group() {
  ...
  if [ "${COMMAND}" == "deploy" ]; then
    ...
    log "Installing chaincode for Rbi"
    install_chaincode $@
    log "🏁 - Chaincode is installed"

  elif [ "${COMMAND}" == "approve" ]; then
    log "Approving chaincode for Rbi"
    approve_chaincode $@
    log "🏁 - Chaincode is approved"

  elif [ "${COMMAND}" == "commit" ]; then
    log "Committing chaincode for Rbi"
    commit_chaincode $@
...
function launch_chaincode_service() {
  ...
  cat kube/${org}/${org}-cc-template.yaml \
    ...
    | exec kubectl -n $Rbi_NS apply -f -

  kubectl -n $Rbi_NS rollout status deploy/${org}${peer}-ccaas-${cc_name}
