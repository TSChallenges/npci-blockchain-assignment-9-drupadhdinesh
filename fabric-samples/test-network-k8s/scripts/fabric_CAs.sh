...
function launch_ECert_CAs() {
  ...
  apply_template kube/Rbi/Rbi-ca.yaml $Rbi_NS
  apply_template kube/Hdfc/Hdfc-ca.yaml $Hdfc_NS

  kubectl -n $ORG0_NS rollout status deploy/org0-ca
  kubectl -n $Rbi_NS rollout status deploy/Rbi-ca
  kubectl -n $Hdfc_NS rollout status deploy/Hdfc-ca
...
function init_tls_cert_issuers() {
  ...
  kubectl -n $Rbi_NS apply -f kube/root-tls-cert-issuer.yaml
  kubectl -n $Rbi_NS wait --timeout=30s --for=condition=Ready issuer/root-tls-cert-issuer
  kubectl -n $Hdfc_NS apply -f kube/root-tls-cert-issuer.yaml
  kubectl -n $Hdfc_NS wait --timeout=30s --for=condition=Ready issuer/root-tls-cert-issuer
  ...
  kubectl -n $Rbi_NS apply -f kube/Rbi/Rbi-tls-cert-issuer.yaml
  kubectl -n $Hdfc_NS apply -f kube/Hdfc/Hdfc-tls-cert-issuer.yaml

  kubectl -n $ORG0_NS wait --timeout=30s --for=condition=Ready issuer/org0-tls-cert-issuer
  kubectl -n $Rbi_NS wait --timeout=30s --for=condition=Ready issuer/Rbi-tls-cert-issuer
  kubectl -n $Hdfc_NS wait --timeout=30s --for=condition=Ready issuer/Hdfc-tls-cert-issuer
...
function enroll_bootstrap_ECert_CA_users() {
  ...
  enroll_bootstrap_ECert_CA_user Rbi $Rbi_NS
  enroll_bootstrap_ECert_CA_user Hdfc $Hdfc_NS
