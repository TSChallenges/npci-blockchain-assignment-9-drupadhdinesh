...
const { buildCCPRbi, buildCCPHdfc, buildWallet, prettyJSONString } = require('../../test-application/javascript/AppUtil.js');
...
async function endAuction (ccp, wallet, org, user, auctionID) {
	try {
		...
		statefulTxn.setEndorsingOrganizations(org, 'SbiMSP');
...
async function main () {
	try {
		...
		if (org === 'Rbi' || org === 'Rbi') {
			const orgMSP = 'RbiMSP';
			const ccp = buildCCPRbi();
			const walletPath = path.join(__dirname, 'wallet/Rbi');
			const wallet = await buildWallet(Wallets, walletPath);
			await endAuction(ccp, wallet, orgMSP, user, auctionID);
		} else if (org === 'Hdfc' || org === 'Hdfc') {
			const orgMSP = 'HdfcMSP';
			const ccp = buildCCPHdfc();
			const walletPath = path.join(__dirname, 'wallet/Hdfc');
			const wallet = await buildWallet(Wallets, walletPath);
			await endAuction(ccp, wallet, orgMSP, user, auctionID);
		} else {
			console.log('Usage: node endAuction.js org userID auctionID');
