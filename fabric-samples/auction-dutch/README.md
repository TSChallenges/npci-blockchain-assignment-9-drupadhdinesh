...
```
./network.sh deployCC -ccn auction -ccp ../auction-dutch/chaincode-go/ -ccep "OR('RbiMSP.peer','HdfcMSP.peer')" -ccl go
```

Note that we deploy the smart contract with an endorsement policy of `"OR('RbiMSP.peer','HdfcMSP.peer')" ` instead of using the default endorsement policy of the majority of orgs on the channel. Either Rbi or Hdfc can create an auction without the endorsement of the other organization.

## Add an auditor (optional)

The smart contract allows you to add an auditor organization to the auction. The auditor can add bids, close the auction, or end the auction if participants cannot cooperate. In this tutorial, we will add the Sbi organization to the test network channel and install an auditor specific version of the dutch auction smart contract. This allows you to use Sbi as the auditor organization.

From the `test-network` directory, issue the following commands to add Sbi to the channel:
...
Set the following environment to interact with the test network as Sbi.
```
...
To deploy the smart contract on the Sbi peer, we need to use the peer lifecycle chaincode commands to install the chaincode package and approve the chaincode definition as Sbi. Run the following command to package the auditor version of the dutch auction smart contract:
```
peer lifecycle chaincode package auction.tar.gz --path ../auction-dutch/chaincode-go-auditor/ --lang golang --label auction_1
```
Install the chaincode package on the Sbi peer:
```
peer lifecycle chaincode install auction.tar.gz
```

The next step is to approve the chaincode as the Sbi admin. This requires getting the package ID of the chaincode that we just installed.
```
...
You can now approve the auction chaincode for Sbi:
```
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" --channelID mychannel --name auction --version 1.0 --package-id $CC_PACKAGE_ID --sequence 1 --signature-policy "OR('RbiMSP.peer','HdfcMSP.peer')"
```

The command will start the dutch auction chaincode on the Sbi peer. Note that we did not update the endorsement policy before we added the auditor organization. Only Rbi and Hdfc will be able create an auction. The auditor is added the endorsement policy after the auction is created. Because the auditor does not need to create an auction or create new bids, the auditor can run a different version of the smart contract than the auction participants. The auditor version of the smart contract also adds logic to check that the request is submitted by one of the auction participants before the auditor can intervene.
...
To interact with the network, you will need to enroll the Certificate Authority administrators of Rbi and Hdfc. You can use the `enrollAdmin.js` program for this task. Run the following command to enroll the Rbi admin:
```
node enrollAdmin.js Rbi
```
You should see the logs of the admin wallet being created on your local file system. Now run the command to enroll the CA admin of Hdfc:
```
node enrollAdmin.js Hdfc
```
...
```
node registerEnrollUser.js Rbi seller
```

You should see the logs of the seller wallet being created as well. Run the following commands to register and enroll two bidders from Rbi and another three bidders from Hdfc:
```
node registerEnrollUser.js Rbi bidder1
node registerEnrollUser.js Rbi bidder2
node registerEnrollUser.js Hdfc bidder3
node registerEnrollUser.js Hdfc bidder4
node registerEnrollUser.js Hdfc bidder5
```

## Create the auction

The seller from Rbi would like to create an auction to sell 100 tickets. Run the following command to use the seller wallet to run the `createAuction.js` application. The seller needs to provide an auction ID, the item to be sold, and the quantity to be sold to create the auction. The seller uses `withAuditor` to indicate that Sbi will be added as the auditor organization. If you do not want to add an auditor, you can provide a value of `noAuditor`. You will see the application query the auction after it is created.
```
node createAuction.js Rbi seller auction1 tickets 100 withAuditor
```

Adding an auditor to the auction creates an endorsement policy with the auditor included. Without the auditor, each organization with sellers or bidders participating in the auction is added to the auction endorsement policy. For example, if the auction had two organizations participating in the auction, the auction endorsement policy would be `AND(Rbi, Hdfc)`. However, if the selling organization decides to add an auditor, the auditor organization would be added to the endorsement policy. If the participating organizations disagree, or if a participant has a technical problem, the auditor can join any one of the participating organizations and agree to update the auction. Extending the example above, if the auction with two organizations added an auditor, the auction endorsement policy would be `OR(AND(Rbi, Hdfc), AND(auditor, OR(Rbi, Hdfc)))`.
...
```
node bid.js Rbi bidder1 auction1 50 80
```
...
*** Result:  Bid: {
  ...
  "price": 80,
  "org": "RbiMSP",
  "buyer": "x509::CN=bidder1,OU=client+OU=Rbi+OU=department1::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US"
}
```
...
```
node submitBid.js Rbi bidder1 auction1 $BIDDER1_BID_ID
```
...
*** Result: Auction: {
  "objectType": "auction",
  "item": "tickets",
  "seller": "x509::CN=seller,OU=client+OU=Rbi+OU=department1::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US",
  "quantity": 100,
  "organizations": [
    "RbiMSP"
  ],
...
```
node bid.js Rbi bidder2 auction1 40 50
```
...
```
node submitBid.js Rbi bidder2 auction1 $BIDDER2_BID_ID
```

### Bid as bidder3 from Hdfc

Bidder3 will bid for 30 tickets at 70 dollars:
```
node bid.js Hdfc bidder3 auction1 30 70
```
...
```
node submitBid.js Hdfc bidder3 auction1 $BIDDER3_BID_ID
```

Because bidder3 belongs to Hdfc, submitting the bid will add Hdfc to the list of participating organizations. You can see the Hdfc MSP ID has been added to the list of `"organizations"` in the updated auction returned by the application:
```
*** Result: Auction: {
  "objectType": "auction",
  "item": "tickets",
...
Now that a bid from Hdfc has been added to the auction, any updates to the auction need to be endorsed by the Hdfc peer. The applications will use the `"organizations"` field to specify which organizations need to endorse submitting a new bid, revealing a bid, or updating the auction status.

### Bid as bidder4

Bidder4 from Hdfc would like to purchase 15 tickets for 60 dollars:
```
node bid.js Hdfc bidder4 auction1 15 60
```
...
```
node submitBid.js Hdfc bidder4 auction1 $BIDDER4_BID_ID
```

### Bid as bidder5

Bidder5 from Hdfc will bid for 20 tickets at 60 dollars:
```
node bid.js Hdfc bidder5 auction1 20 60
```
...
```
node submitBid.js Hdfc bidder5 auction1 $BIDDER5_BID_ID
```
...
```
node closeAuction.js Rbi seller auction1
```
...
```
node revealBid.js Rbi bidder1 auction1 $BIDDER1_BID_ID
```
...
```
We will add three more bidders, the second bidder from Rbi and two bidders from Hdfc. Run the following commands to reveal the bidders:
```
node revealBid.js Rbi bidder2 auction1 $BIDDER2_BID_ID
node revealBid.js Hdfc bidder4 auction1 $BIDDER4_BID_ID
node revealBid.js Hdfc bidder5 auction1 $BIDDER5_BID_ID
```

Let's try to end the auction using the seller identity and see what happens.

```
node endAuction.js Rbi seller auction1
```
...
Instead of ending the auction, the transaction results in an endorsement policy failure. The end of the auction needs to be endorsed by Hdfc. Before endorsing the transaction, the Hdfc peer queries its private data collection for any winning bids that have not yet been revealed. Because the price that would clear the auction with the currently revealed bids is lower than the bid of Bidder3, the Hdfc peer refuses to endorse the transaction that would end the auction.

In order to end the auction, Rbi would either need to wait for Hdfc to reveal the final bid or appeal to the auditor. Depending on if you created the organization with an auditor, you can end the auction with either set of steps.

## End the auction using an auditor

If Hdfc is unable to endorse the transaction to end the auction, Rbi can ask the auditor to intervene. The following program gets an endorsement from the Sbi auditor and Rbi to end the auction. As a result, the transaction would meet the auditor component of the state based endorsement policy.
```
node endAuctionwithAuditor Rbi seller auction1
```

Even though Hdfc has not agreed to the end of the auction, the endorsement Rbi is sufficient to end the auction if the auditor agrees. As part of ending the auction, both Rbi and the auditor need to calculate the same price and the same set of winners. Each winning bidder is listed next to the quantity that was allocated to them.
```
*** Result: Auction: {
  "objectType": "auction",
  "item": "tickets",
  ...
  "revealedBids": {
    "\u0000bid\u0000auction1\u00005796569dae2e95242eadc5cf1cf8aa24f5ae072d801e7decb2547530de5a65e8\u0000": {
      ...
      "price": 50,
      "org": "RbiMSP",
      "buyer": "x509::CN=bidder2,OU=client+OU=Rbi+OU=department1::CN=ca.Rbi.example.com,O=Rbi.example.com,L=Durham,ST=North Carolina,C=US"
    },
    "\u0000bid\u0000auction1\u00006630e1bb06e827a2b77023f63677fae8a0ad43126730e450d3252fa58eeb85b1\u0000": {
      "objectType": "bid",
      "quantity": 50,
    ...
    "\u0000bid\u0000auction1\u0000c6464f984bb01e639a46e58b94c496e8bbd829b5e4fa7ffcc150d9a565d45684\u0000": {
      ...
      "price": 60,
      "org": "HdfcMSP",
      "buyer": "x509::CN=bidder4,OU=client+OU=Hdfc+OU=department1::CN=ca.Hdfc.example.com,O=Hdfc.example.com,L=Hursley,ST=Hampshire,C=UK"
    },
    "\u0000bid\u0000auction1\u0000f4024ab09b4dacf0a636927414850dde2a2a5e8ec4601e2a0071f5c233248207\u0000": {
      "objectType": "bid",
      "quantity": 20,
      "price": 60,
      "org": "HdfcMSP",
      "buyer": "x509::CN=bidder5,OU=client+OU=Hdfc+OU=department1::CN=ca.Hdfc.example.com,O=Hdfc.example.com,L=Hursley,ST=Hampshire,C=UK"
    }
  },
  "winners": [
    {
...
If we did not add an auditor to the auction, we need to add the remaining bid so that Hdfc will endorse ending the auction.
```
node revealBid.js Hdfc bidder3 auction1 $BIDDER3_BID_ID
```

Now that all the winning bids have been revealed, we can submit the transaction to end the auction once more.
```
node endAuction Rbi seller auction1
```

The transaction was successfully endorsed by both Rbi and Hdfc, who both calculated the same price and winners of the auction. Each winning bidder is listed next to the quantity that was allocated to them.
```
*** Result: Auction: {
  "objectType": "auction",
  "item": "tickets",
  ...
  "revealedBids": {
    ...
    "\u0000bid\u0000auction1\u000048d93017ac65cff0dd23406cc29918724fd84c8e7014eee30fd492fef760e6a4\u0000": {
      ...
      "price": 70,
      "org": "HdfcMSP",
      "buyer": "x509::CN=bidder3,OU=client+OU=Hdfc+OU=department1::CN=ca.Hdfc.example.com,O=Hdfc.example.com,L=Hursley,ST=Hampshire,C=UK"
    },
    "\u0000bid\u0000auction1\u00005ba4c856224cdc8209b0e42f30a757331e3fb8a8b660b64a55e1bcf688b745ad\u0000": {
      "objectType": "bid",
      "quantity": 40,
