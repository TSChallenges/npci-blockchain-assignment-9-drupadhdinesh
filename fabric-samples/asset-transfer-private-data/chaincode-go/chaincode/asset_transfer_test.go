...
const myRbiMSP = "RbiTestmsp"
const myRbiClientid = "myRbiUserid"
const myRbiPrivCollection = "RbiTestmspPrivateCollection"
const myHdfcMSP = "HdfcTestmsp"
const myHdfcClientid = "myHdfcUserid"
const myHdfcPrivCollection = "HdfcTestmspPrivateCollection"
...
func TestCreateAssetSuccessful(t *testing.T) {
	...
	require.Equal(t, myRbiPrivCollection, calledCollection)
	require.Equal(t, "id1", calledId)
...
func TestAgreeToTransferSuccessful(t *testing.T) {
	...
	require.Equal(t, []byte(myRbiClientid), calledWithDataBytes)
}
...
func TestTransferAssetSuccessful(t *testing.T) {
	...
	// to ensure that ReadTransferAgreement call returns Hdfc client ID
	chaincodeStub.GetPrivateDataReturnsOnCall(1, []byte(myHdfcClientid), nil)
	chaincodeStub.CreateCompositeKeyReturns(transferAgreementObjectType+"id1", nil)
	...
	expectedNewAsset.Owner = myHdfcClientid
...
func TestTransferAssetByNonOwner(t *testing.T) {
	...
	setReturnAssetOwnerInTransientMap(t, chaincodeStub, assetNewOwner)
	// Try to transfer asset owned by Hdfc
	HdfcAsset := chaincode.Asset{
		ID:    "id1",
		Type:  "testfulasset",
		Color: "gray",
		Size:  7,
		Owner: myHdfcClientid,
	}
	setReturnPrivateDataInStub(t, chaincodeStub, &HdfcAsset)
	err := assetTransferCC.TransferAsset(transactionContext)
...
func prepMocksAsRbi() (*mocks.TransactionContext, *mocks.ChaincodeStub) {
	return prepMocks(myRbiMSP, myRbiClientid)
}
func prepMocksAsHdfc() (*mocks.TransactionContext, *mocks.ChaincodeStub) {
	return prepMocks(myHdfcMSP, myHdfcClientid)
}
