...
func TestReadAsset(t *testing.T) {
	transactionContext, chaincodeStub := prepMocksAsRbi()
...
func TestReadAssetPrivateDetails(t *testing.T) {
	...
	assetBytes, err := assetTransferCC.ReadAssetPrivateDetails(transactionContext, myRbiPrivCollection, "id1")
	require.NoError(t, err)
	require.Nil(t, assetBytes)

	// read from the collection with no access
	chaincodeStub.GetPrivateDataReturns(nil, fmt.Errorf("collection not found"))
	assetBytes, err = assetTransferCC.ReadAssetPrivateDetails(transactionContext, myHdfcPrivCollection, "id1")
	...
	assetRead, err := assetTransferCC.ReadAssetPrivateDetails(transactionContext, myRbiPrivCollection, "id1")
	require.NoError(t, err)
...
func TestReadTransferAgreement(t *testing.T) {
	...
	chaincodeStub.GetPrivateDataReturns([]byte(myHdfcClientid), nil)
	expectedData := &chaincode.TransferAgreement{
		ID:      "id1",
		BuyerID: myHdfcClientid,
	}
