...
const { buildCCPRbi, buildWallet } = require('../../test-application/javascript/AppUtil.js');
...
async function main() {
	...
	try {
		...
		const caClient = buildCAClient(FabricCAServices, ccp, 'ca.Rbi.example.com');

		// setup the wallet to hold the credentials of the application user
		const wallet = await buildWallet(Wallets, walletPath);

		// in a real application this would be done on an administrative flow, and only once
		await enrollAdmin(caClient, wallet, mspRbi);

		// in a real application this would be done only when a new user was required to be added
		// and would be part of an administrative flow
		await registerAndEnrollUser(caClient, wallet, mspRbi, userId, 'Rbi.department1');
